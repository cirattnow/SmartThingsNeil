...
def parse(description) {
	...
	if (description?.startsWith("read attr -")) {
		...
		log.debug( "zigbee.parseDescriptionAsMap(description): " +  groovy.json.JsonOutput.prettyPrint(groovy.json.JsonOutput.toJson(zigbee.parseDescriptionAsMap(description))));
		log.debug( "zigbee.getEvent(description): " +  groovy.json.JsonOutput.prettyPrint(groovy.json.JsonOutput.toJson(zigbee.getEvent(description))));
		if (zigbee.convertHexToInt(descMap.cluster) == zigbee.THERMOSTAT_CLUSTER && zigbee.convertHexToInt(descMap.attrId) == 0) {
